...
def processamento_base(dados: pd.DataFrame) -> list[list[list]]:
    ...
    onehotencoder = ColumnTransformer(transformers=[('OneHot', OneHotEncoder(), [0, 2, 4, 5, 9, 10])],
...
def previsao(idade:int, renda:float, imovel:str, tempo_trabalho:float, intencao:str, grau:str, valor_emprestimo:float,
             taxa_emprestimo:float,taxa_rendimento:float, historico_inad:str, historico_credito:int) -> bool:
    '''Faz a previsão do risco de crédito.'''
    nova_linha = {'person_age': [idade], 'person_income': [renda], 'person_home_ownership': [imovel],
                  'person_emp_length': [tempo_trabalho], 'loan_intent': [intencao], 'loan_grade': [grau],
                  'loan_amnt': [valor_emprestimo], 'loan_int_rate': [taxa_emprestimo],
                  'loan_percent_income': [taxa_rendimento], 'cb_person_default_on_file': [historico_inad],
...
def previsao_grau_risco(idade:int, renda:float, imovel:str, tempo_trabalho:float, intencao:str, valor_emprestimo:float, taxa_emprestimo:float,
                        taxa_rendimento:float, historico_inad:str, historico_credito:int) -> bool:
    '''Faz a previsão do grau de risco do empréstimo.'''
    nova_linha = {'person_age': [idade], 'person_income': [renda], 'person_home_ownership': [imovel],
                  'person_emp_length': [tempo_trabalho], 'loan_intent': [intencao],'loan_amnt': [valor_emprestimo],
                  'loan_int_rate': [taxa_emprestimo],'loan_percent_income': [taxa_rendimento],
                  'cb_person_default_on_file': [historico_inad], 'cb_person_cred_hist_length': [historico_credito]}
    nova_linha = pd.DataFrame(nova_linha)
...
class PDF(FPDF):
    ...
    def footer(self):
        ...
        self.cell(0, 10, str(datetime.now().strftime('%Y-%m-%d %H:%M:%S')), 1, 0, 'C')
...
def gera_pdf(dados:pd.DataFrame, colunas:list, termo:str) -> None:
    ...
    for linha in range(0, len(dados)):
        pdf.cell(w=95, h=10, txt=dados[colunas[0]].iloc[linha], border=1, ln=0, align='L')
        pdf.cell(w=95, h=10, txt=str(dados[colunas[1]].iloc[linha]), border=1, ln=1, align='L')
...
def main():
    ...
    st.markdown("<h2 style='text-align: center; font-family: Verdana'>CreditInspector ML</h2>", unsafe_allow_html=True,)
    st.header('', divider='blue')
    ...
    # Idade
    data_nascimento = st.date_input('Data de Nascimento:', value=None, min_value=datetime(idade_ano(60), 12, 31), 
                                    max_value=datetime(idade_ano(20),12,31), format='DD/MM/YYYY')
    ...
    # Tipo de residência
    tipo_imovel = st.selectbox('Tipo de Residência:', ['Própria', 'Financiada', 'Alugada', 'Outros'])
    ...
    tipo_intencao = st.selectbox('Finalidade do Empréstimo:', ['Educação', 'Despesas Médicas', 
                                                               'Empréstimo Empresarial','Pessoal',
    ...
    # Taxa de juros
    taxa_emprestimo = st.slider('Taxa de Juros Mensal do Empréstimo (%):', 0.45, 1.93, 0.5)
 
    # Total de parcelas
    quantidade_parcelas = st.selectbox('Quantidade de Parcelas: ', ['6x', '12x', '24x', '36x', '48x', '60x', '72x'])
    ...
    valor_parcelas = financiamento_price(valor_emprestimo, parcelas, (taxa_emprestimo/100))
    taxa_emprestimo_anual = round((taxa_emprestimo * 12), 2)

    # Taxa de renda para o emprestimo
    try:
        if renda != 0:
            taxa_rendimento_automatica = round((valor_parcelas / renda_mensal) * 100,2)
            taxa_rendimento_automatica_formatada = '{:.2f}%'.format(taxa_rendimento_automatica)
            if taxa_rendimento_automatica != 0:
                st.write('Comprometimento da Renda com Empréstimo:', str(taxa_rendimento_automatica_formatada))
            else:
                raise ZeroDivisionError('O valor do Empréstimo não pode ser 0.00.')
        else:
            raise ZeroDivisionError('O valor da Renda Mensal não pode ser 0.00.')
    ...
    try:
        if nome and cpf is not None:
            valor_parcelas_formatado = "{:,.2f}".format(valor_parcelas).replace(',', ' ').replace('.',',').replace(' ', '.')   
    ...
    tipo_inadimplencia = st.selectbox('Histórico de Inadimplência:', ['SIM', 'NÃO'])
    ...
    historico_credito = st.slider('Histórico de Crédito (em anos):', 2, 30, 2, )

    # Grau de risco do empréstimo
    grau = ''
    try:
        if idade is not None and renda and valor_emprestimo is not 0:
            grau = previsao_grau_risco(idade, renda, imovel, tempo_trabalho, intencao, valor_emprestimo, taxa_emprestimo_anual,
                                    taxa_rendimento_automatica, historico_inadimplencia, historico_credito)
    ...
    if st.button('ANALISAR CRÉDITO'):
        try:
            if nome and cpf is not idade and renda and valor_emprestimo is not 0:
                if 20 <= idade <= 60 and taxa_rendimento_automatica < 40:
                    resultado = previsao(idade, renda, imovel, tempo_trabalho, intencao,
                                        grau,valor_emprestimo,taxa_emprestimo_anual, taxa_rendimento_automatica,
                                        historico_inadimplencia, historico_credito)
                    
                    if resultado != 1:
                        valor = valor_emprestimo
                        valor_formatado = "{:,.2f}".format(valor).replace(',', ' ').replace('.',',').replace(' ', '.')
                        mensagem = f'Linha de crédito de {valor_formatado} liberada.'
                        st.success(mensagem, icon='✔')

                        st.write('Proposta de Empréstimo:')
                        ficha = {
                            'Nome do Cliente':nome,'CPF':cpf, 'Idade': idade, 'Renda Mensal':renda_mensal,
                            'Tipo de Residência':tipo_imovel,'Tempo de Emprego':tempo_trabalho,'Finalidade do Empréstimo':tipo_intencao,
                            'Grau de Empréstimo':tipo_grau,'Valor do Emprestimo':valor_emprestimo,'Total de Parcelas':parcelas,
                            'Taxa de Juros Mensal (%)':taxa_emprestimo,'Valor das Parcelas':valor_parcelas_formatado,
                            'Percentual de Renda p/ Empréstimo':taxa_rendimento_automatica_formatada,
                            'Histórico de Inadimplência':tipo_inadimplencia,'Histórico de Crédito (em anos)':historico_credito
                        }
